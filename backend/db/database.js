...
async function inicializar() {
  try {
    console.log('⏳ Iniciando tablas en PostgreSQL...');
    ...
    if (rows.length === 0) {
      ...
      await query(
        `INSERT INTO usuarios (nombre, apellido, email, password_hash, rol, estado) 
    ...
    console.error('❌ Error en inicializar DB:', error.message);
    // No lanzamos el error para evitar que el servidor se caiga infinitamente, 
    // pero el log te dirá qué pasó.
  }
}

// Exportamos pool también por si necesitas cerrar conexiones luego
